...
contract USDT is ERC20 {
    ...
    constructor() ERC20("USDToken", "USDTokenSymbol") {
        _mint(msg.sender, 40000000 * 1e18); // mint 40m to team
    }
}
...
contract Staking is Ownable {
    ...
    uint256 public immutable blocks_per_day = 28800;
    uint256 public immutable seconds_per_day = 86400;
    uint256 public Friday = 1675382400; // this is the 1st Friday
    uint256 public ActionDay = 1675382400; // this is a specific day (starts with Friday)
    address NFTaddress; // this is the OG NFT contract address
    address NFTaddress2; // this is the Whitelist NFT contract address
    address USDTaddress;
    IERC20 public USDT;
    ...
    struct UserInfo {
        ...
        uint256 TotaDeposits; // Total USDT the user has provided
    ...
    event SetFees(uint256 depositFeeBP, uint256 withdrawFeeBP, uint256 compoundFeeBP);
    event UserWithdraw(address indexed user, uint256 amount);
    event WithdrawInitiated(address indexed user, uint256 unlockDay);

    constructor(address _NFTaddress, address _NFTaddress2, address _USDTaddress) {
        feeWallet = msg.sender;
        EmergencyfeeWallet = msg.sender;
        USDT = IERC20(_USDTaddress);
        NFTContract = myNFT(_NFTaddress);
        NFTContract2 = myNFT(_NFTaddress2);
    }

    modifier hasNFT(address user) {
        require(NFTContract.balanceOf(user) > 0 || NFTContract2.balanceOf(user) > 0, "User doesn't own NFT");
        _;
    }

    modifier onlyActionDay() {
        require(getDifferenceFromActionDay() == 0 || getDifferenceFromActionDay() == 7, "wrong day");
        _;
    }
    ...
    function deposit(uint256 _amount) external hasStarted hasNFT(msg.sender) {
        ...
        user.deposits.push(
            Depo({ amount: _amount - depositFee, time: block.timestamp, lastActionTime: 0, unlocked: 0, isCompound: 0 })
        );

        USDT.safeTransferFrom(address(msg.sender), address(this), _amount - depositFee);

        USDT.safeTransferFrom(address(msg.sender), EmergencyfeeWallet, depositFee);

        user.TotaDeposits += _amount;
        user.NoOfDeposits += 1;
        invested += _amount;
        withdrawn += depositFee;
     ...
     */
    function Initiatecompound() external onlyActionDay {
        UserInfo storage user = userInfo[msg.sender];
        require(
            user.WithdrawInitiated == 0 && user.ClaimInitiated == 0 && user.CompoundInitiated == 0,
            "Action already initialised"
        );
        require(block.timestamp > user.lastRewardTimeStamp + 6 days, "already interacted last 2 week");
        user.CompoundInitiated = 1;
        user.lastRewardTimeStamp = block.timestamp;
    }

    /*
     * @notice function to initiate a claim of all eligible deposits.
     */
    ...
    function Claim() external onlyActionDay returns (uint256 claimed) {
        UserInfo storage user = userInfo[msg.sender];
        require(
            user.WithdrawInitiated == 0 && user.ClaimInitiated == 1 && user.CompoundInitiated == 0,
            "Claim Not initialised"
        );
        require(block.timestamp > user.lastRewardTimeStamp + 6 days, "already interacted last 2 week");
        ...
        for (uint256 i; i < NoOfDeposits; ) {
            ...
            if (
                (dep.isCompound == 0 || dep.unlocked != 1) && // deposit or relocked compound
                block.timestamp > dep.time + 39 days && // deposits only earn 1st friday after 31 days + 8 (7 min to initiate claim)
                block.timestamp - dep.lastActionTime > 7 days
            ) {
                // get the right reference day to calculate rewards.
                // user.lastRewardTimeStamp is last time user compounded/claimed
                // dep.time is deposit time

                uint256 period = 7 days; //because min and max are 7
                uint256 multiplier = (period) / 3;
                uint256 rewardperblock = (dep.amount * DROP_RATE) / blocks_per_day / 10000;
                claimed += (multiplier * rewardperblock);
        ...
        USDT.safeTransfer(feeWallet, withdrawFee);
        USDT.safeTransfer(user.WithdrawAddress, claimed - withdrawFee);
        user.lastRewardTimeStamp = block.timestamp;
        user.ClaimInitiated = 0;
        withdrawn += claimed - withdrawFee;
     ...
     */
    function Compound() external onlyActionDay returns (uint256 compoundedAmount) {
        UserInfo storage user = userInfo[msg.sender];
        require(
            user.WithdrawInitiated == 0 && user.ClaimInitiated == 0 && user.CompoundInitiated == 1,
        ...
        for (uint256 i; i < NoOfDeposits; ) {
            ...
            if (
                (dep.isCompound == 0 || dep.unlocked != 1) && // deposit or relocked compound
                block.timestamp > dep.time + 39 days && // deposits only earn 1st friday after 31 days + 8 (7 min to initiate claim)
                block.timestamp - dep.lastActionTime > 6 days
            ) {
                // get the right reference day to calculate rewards.
                // user.lastRewardTimeStamp is last time user compounded/claimed
                // dep.time is deposit time
                ...
                uint256 period = 7 days; //because min and max are 7
                uint256 multiplier = (period * 10 ** 18) / 3;
                uint256 rewardperblock = (dep.amount * DROP_RATE) / blocks_per_day / 10000;

                pending += (multiplier * rewardperblock) / 10 ** 18;
                dep.lastActionTime = block.timestamp;
            }
        ...
        user.deposits.push(
            Depo({ amount: compoundedAmount, time: block.timestamp, lastActionTime: 0, unlocked: 0, isCompound: 1 })
        );
        USDT.safeTransfer(feeWallet, compoundFee);
        user.lastRewardTimeStamp = block.timestamp;
        user.NoOfDeposits += 1;
        user.CompoundInitiated = 0;
        user.TotalReinvested += compoundedAmount;
        reinvested += compoundedAmount;
    ...
    /*
     * @notice function to initiate withdraw of deposits
     */
    function initiateWithdraw() external onlyActionDay {
        UserInfo storage user = userInfo[msg.sender];
        require(
            user.WithdrawInitiated == 0 && user.ClaimInitiated == 0 && user.CompoundInitiated == 0,
            "Action already initialised"
        ...
        emit WithdrawInitiated(msg.sender, block.timestamp + 60 days);
    }

    /*
     * @notice function to withdraw deposits.
     */
    function Withdraw() external returns (uint256 finalAmount, uint256 fee) {
        UserInfo storage user = userInfo[msg.sender];
        require(user.WithdrawInitiated == 1, "Withdraw not intiated");
        require(user.WithdrawDate > 0 && user.WithdrawDate < block.timestamp, "Withdraw not yet");
        (finalAmount) = returnFinalAmount();
        // max withdraw is 10k USDT, if excess then create new Compounded Deposit
        ...
        USDT.safeTransfer(feeWallet, fee);
        USDT.safeTransfer(user.WithdrawAddress, finalAmount - fee);
     ...
     * @param _newRate: 10000 is 10k USDT
     */
     ...
     * @param _depositFee,  100 is 1%, 200 is 2% etc
     */
    function changeFees(uint256 _depositFeeBP, uint256 _withdrawFeeBP, uint256 _compoundFeeBP) external onlyOwner {
        require(_depositFeeBP != 0 && _withdrawFeeBP != 0 && _compoundFeeBP != 0, "Fees cannot be zero");
     ...
     */
    function ChangeEmergencyfeeAddress(address _EmergencyfeeWallet) external onlyOwner {
     ...
     * @return no. of week since start
     */
    function getWeek() public view returns (uint256 totalweeks) {
        return (block.timestamp - Friday) / seconds_per_day / (7);
    }

    /*
     * @notice function to change action day.
     * @param _ActionDay: day of action, 0 = friday, 1 = Saturday
     */
    function changeActionDay(uint256 _ActionDay) external onlyOwner {
        require(_ActionDay < 14, "0 to 13 allowed only");
        ActionDay = Friday + _ActionDay * seconds_per_day;
    }
     ...
     */
    function getDayFromActionDay() external view returns (uint256) {
        uint256 totalsec = (block.timestamp - ActionDay); //total sec from friday
        return totalsec / seconds_per_day;
    }
    ...
    function getDifferenceFromActionDay() public view returns (uint256) {
        uint256 totalsec = (block.timestamp - ActionDay); //total sec from friday
        return totalsec / seconds_per_day - getWeek() * 7; //7 days in a week
    }
     ...
     */
    function decideDeposit(uint256 _depo, uint256 _decision) public {
        UserInfo storage user = userInfo[msg.sender];
    ...
    function pendingRewards() public view returns (uint256 USDTReward) {
        ...
        for (uint256 i; i < NoOfDeposits; ) {
            ...
            if (
                // deposit or relocked compound
                (dep.isCompound == 0 || dep.unlocked != 1) &&
                dep.unlocked != 1 &&
                // deposits only earn 1st friday after 31 days + 8 (7 min to initiate claim)
                block.timestamp > dep.time + 31 days
            ) {
                // get the right reference day to calculate rewards.
                // user.lastRewardTimeStamp is last time user compounded/claimed
                // dep.time is deposit time
                ...
                USDTReward += (multiplier * rewardperblock);
            }
     ...
     * @return Pending reward for a given user
     */
    function pendingReward(uint256 _deposit) public view returns (uint256 USDTReward) {
        UserInfo storage user = userInfo[msg.sender];
    ...
    function returnFinalAmount() public view returns (uint256 finalAmount) {
        ...
        finalAmount = pendingRewards() + user.TotaDeposits - user.initialDeposit;

        return (finalAmount);
    }

    /*
     * @notice View function to see 30d timer to deposit of user deposits.
     * @return time : time to 30d
     */
    function depositCounter30d(address _addr, uint256 index) external view returns (uint256 time) {
        UserInfo storage user = userInfo[_addr];
        Depo storage dep = user.deposits[index];

        if (dep.time + 30 days > block.timestamp) {
            return (dep.time + 30 days) - block.timestamp;
        }
        return 0;
    }

    /*
     * @notice View function to see 60d timer to deposit of user deposits.
     * @return time : time to 60d
     */
    function depositCounter60d(address _addr, uint256 index) external view returns (uint256 time) {
        UserInfo storage user = userInfo[_addr];
        Depo storage dep = user.deposits[index];

        if (dep.time + 60 days > block.timestamp) {
            return (dep.time + 60 days) - block.timestamp;
        }
        return 0;
    }
     ...
     */
    function memberDeposit(address _addr, uint256 index) external view returns (Depo memory dep) {
        UserInfo storage user = userInfo[_addr];
